...
fn main() -> ! {
    ...
    let usb_serial = arduino_hal::default_serial!(dp, pins, 115200);
    ...
    loop {
        ...
        match MODE {
            ...
            LiketrainMode::Slave { slave_id } => {
                ...
                while let Some(slave_command) = rs485.read::<SlaveCommand>().ok().flatten() {
                    match slave_command {
                        ...
                        SlaveCommand::EventPoll {
                            ...
                            for event in execution_ctx.event_list.drain(..) {
                                let _ = rs485.write::<SlaveResponse>(&event.into());
                            }
                        }
                        _ => {}
                    }
        ...
        if MODE.is_master() {
            for command in slave_commands.drain(..) {
                let _ = rs485.write::<SlaveCommand>(&command.into());
            }
        }
        ...
        if MODE.is_master() {
            for slave_id in &slave_ids {
                ...
                for _ in 0..event_count {
                    ...
                    execution_ctx.event_list.push(event);
                }
            }
        }
        ...
        match MODE {
            LiketrainMode::Master => {
                for event in execution_ctx.event_list.drain(..) {
                    let _ = usb_serial.write(&HardwareResponse::Event(event));
                }
